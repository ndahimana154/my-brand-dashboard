document.getElementById("loginForm").addEventListener("submit", async function (event) {
  ...
  if (!usernameError.textContent && !passwordError.textContent) {
    try {
      ...
      loadingDiv.style.display = "none"; // Hide loading rings after the login request is processed
    }
  } else {
    loadingDiv.style.display = "none"; // Hide loading rings if there are validation errors
  }
});
